...
STACK_NAME=my-cluster-stack
KEY_PAIR_NAME=jc-udacity-devops
#------------------------------------------------------------------------------#
...
echo -e "\n$COL> getting Control Plane ARN...$NOC"
arn_control=$(aws cloudformation describe-stacks \
  "$@" \
  --stack-name "$STACK_NAME" \
  --query "Stacks[0].Outputs[?OutputKey=='ControlPlaneRoleArn'].OutputValue" \
  --output text)
